...
Texture::Texture(VkCommandBuffer cmd,
                 uint32_t width, uint32_t height,
                 VkFormat format,
                 const void* pixels, // Это поле будет nullptr для карты глубины
                 VkImageUsageFlags usage) // Новый аргумент
: width{width}, height{height}, format{format} {
    VkDevice& device = veekay::app.vk_device;
    VkPhysicalDevice& physical_device = veekay::app.vk_physical_device;

    // Для текстур глубины мип-уровни не нужны
    uint32_t mips = 1;

    {
        VkImageCreateInfo info{
            ...
            .samples = VK_SAMPLE_COUNT_1_BIT,
            .tiling = VK_IMAGE_TILING_OPTIMAL,
            .usage = usage, // ИСПОЛЬЗУЕМ НОВЫЙ АРГУМЕНТ usage
            .sharingMode = VK_SHARING_MODE_EXCLUSIVE,
            .initialLayout = VK_IMAGE_LAYOUT_UNDEFINED,
        };
...

    {
        ...
        for (uint32_t i = 0; i < properties.memoryTypeCount; ++i) {
            const VkMemoryType& type = properties.memoryTypes[i];
            if ((requirements.memoryTypeBits & (1 << i)) && (type.propertyFlags & flags) == flags) {
                index = i;
                break;
            }
        }
    ...
    // Определяем, это текстура цвета или глубины
    ...
    // Если pixels == nullptr (как для карты глубины), то копировать данные не нужно
    if (pixels == nullptr) {
        staging = nullptr;
        return;
    }

    // Этот код ниже идентичен старому конструктору, он выполнится только если pixels не пустой
    uint32_t bytes_per_pixel;
    switch (format) {
		case VK_FORMAT_R32G32B32A32_SFLOAT:
			bytes_per_pixel = 16;
    ...
    staging = new Buffer(width * height * bytes_per_pixel, pixels, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
    ...
    dst_to_src_to_sample.subresourceRange.baseMipLevel = mips - 1;
    vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &dst_to_src_to_sample);
}
