...
2. Vertex Shader
   ...
   У меня это происходит в shader.vert
3. Rasterization
   GPU берет 3 обработанные вершины, которые образуют треугольник, и определяет, какие пиксели на экране попадают внутрь. На этом этапе он также интерполирует (плавно изменяет) данные, которые мы получили от вершинного шейдера (например, нормаль f_normal) для каждого пикселя. Если у одной вершины нормаль смотрит вверх, а у другой вбок, то у пикселя посередине нормаль будет смотреть по диагонали.
