...
void update(double time) {
    ...
    if (ImGui::BeginTabBar("LightsTabs")) {
        ...
        // --- ВКЛАДКА: ТОЧЕЧНЫЕ ИСТОЧНИКИ ---
        if (ImGui::BeginTabItem("Point Lights")) {
            
            // Кнопка добавления сверху
            if (point_lights.size() < max_point_lights) {
                if (ImGui::Button("+ Add New Bulb", ImVec2(-1, 0))) { // Кнопка на всю ширину
                    point_lights.push_back(PointLight{
                        .position = camera.position // Создаем там, где камера
                    });
                }
            } else {
                ImGui::TextDisabled("Max lights reached (%d)", max_point_lights);
            }
            ...
            for (int i = 0; i < (int)point_lights.size(); ++i) {
                ImGui::PushID(i); // Чтобы ImGui не путал элементы
                if (ImGui::CollapsingHeader(("Lamp #" + std::to_string(i + 1)).c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
                    ImGui::DragFloat3("Pos", &point_lights[i].position.x, 0.1f);
                    ImGui::SliderFloat("Power", &point_lights[i].intensity, 0.0f, 50.0f);
                    ImGui::ColorEdit3("Color", &point_lights[i].color.x);
                    
                    if (ImGui::Button("Delete Lamp", ImVec2(100, 20))) {
            ...
            // Удаление (с конца, чтобы не побить индексы)
